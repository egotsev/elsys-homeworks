...
class Cell {
    ...
    Path *draw (int length) const {
        ...
        if (has_wall(DOWN)) {
            path->add(Path::Option("L", col_*length + length));
        } else {
            path->add(Path::Option("M", col_*length + length));
        }
        ...
        path->add(Path::Option("", row_*length + length));

        if (has_wall(UP)) {
            path->add(Path::Option("L", col_*length));
        } else {
            path->add(Path::Option("M", col_*length));
        }
        ...
        path->set_property("stroke-width", "3");
...
class Board {
    ...
    void draw(int size = 10) const {
        Canvas canvas(size*cols_, size* rows_);
