...
public:
  ...
  void move() const
  {
    cout << "<path d=\"" << content_ << "\"  stroke=\"black"
         << "\" />" << endl;
  }
};
...
public:
    ...
    m_path << "M" << col_*step << " " << row_*step << " " << "z" << " ";


    m_path << (has_wall(DOWN)?"l":"m") << step << " " << 0 << " " << "z" << " ";

    m_path << (has_wall(RIGHT)?"l":"m") << 0 << " " << step << " "<< "z" << " ";



    m_path << (has_wall(UP)?"l":"m") << -step << " " << 0 << " "<< "z" << " ";

    m_path << (has_wall(UP)?"l":"m") << 0 << " " << -step << " "<< "z" << " ";

    Path main_path(m_path.str());
    main_path.move();
